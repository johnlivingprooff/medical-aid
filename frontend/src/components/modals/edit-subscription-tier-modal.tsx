...
import type { SubscriptionTier, BenefitCategory } from '@/types/models'
...
export function EditSubscriptionTierModal({ open, onOpenChange, tier, scheme, onSave }: Props) {
  ...
  useEffect(() => {
    ...
    setMonthlyPrice(tier.monthly_price.toString())
    setYearlyPrice(tier.yearly_price.toString())
    setMaxDependents(tier.max_dependents.toString())
    setMaxClaimsPerMonth(tier.max_claims_per_month?.toString() || '')
    setMaxCoveragePerYear(tier.max_coverage_per_year?.toString() || '')
    setSortOrder(tier.sort_order.toString())
    setIsActive(tier.is_active)
    
    // Set selected benefit categories
    if (tier.benefit_categories) {
      setSelectedBenefitCategories(tier.benefit_categories.map(cat => cat.id))
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await api.put(`/api/schemes/subscription-tiers/${tier.id}/`, tierData)
      onSave()
      onOpenChange(false)
    } catch (err: any) {
  ...
  async function handleDelete() {
    ...
    if (!confirm(`Are you sure you want to delete the "${tier.name}" subscription tier? This action cannot be undone.`)) {
      return
    }
    ...
    try {
      await api.delete(`/api/schemes/subscription-tiers/${tier.id}/`)
      onSave()
      onOpenChange(false)
    } catch (err: any) {
  ...
  if (!open || !tier || !scheme) return null
    ...
    >
      ...
      >
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle>Edit Subscription Tier</CardTitle>
